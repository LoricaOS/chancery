...
#[derive(Parser)]
#[command(name = "chancery", version, about = "Debian-style signed repo manager for herald")]
struct Cli {
...
enum Cmd {
    ...
    Promote { name: String, from: String, to: String },
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::Add { package, suite, component } => repo::add(&cli.repo, &package, &suite, &component),
        Cmd::Remove { name, version, suite } => {
        ...
        Cmd::Serve { port } => serve::serve(&cli.repo, port),
    }
}
